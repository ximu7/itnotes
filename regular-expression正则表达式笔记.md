...
- 特殊字符：在正则表达式中具有特殊意义的字符，如`\` `.` `[ ]` `?` `*` `+`等。eg-- `\[`  匹配到了 [
- 空白元字符：匹配空白字符，见下表
...
eg--匹配html文件中的h1-h6，如果使用`<[hH][1-6]>.*?</[hH][1-6]>` 则会匹配到 `<h1>一级标题</h2>` 这种情况；利用回溯:`<[hH]（[1-6]）>.*?</[hH]\1>` ，将会在在`</[hH]` 后引用前面已经匹配好的数字，达到前后一致。
...
# **前后查找**
包含匹配的本身**仅用于确定正确的匹配位置**，匹配本身并**不作为匹配结果**的一部分返回。
